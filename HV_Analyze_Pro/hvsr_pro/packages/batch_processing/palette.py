...
Example
-------
>>> from hvsr_pro.packages.batch_processing.palette import load_palette
...
def _fallback_palette() -> List[Tuple[float, float, float]]:
    ...
    return [to_rgb(c) for c in default_cycle]
...
def load_palette(mat_file: Path | str) -> List[Tuple[float, float, float]]:
    ...
    mat_file : str or Path
        .mat file containing an N x 3 array named *Colors* or *Kcolors*.
