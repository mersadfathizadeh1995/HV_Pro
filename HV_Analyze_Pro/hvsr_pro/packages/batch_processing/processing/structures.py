...
class Peak:
    ...
    Attributes:
        ...
        peak_type: Peak classification ("primary", "secondary", or "")
    """
    frequency: float
    amplitude: float
...
class HVSRResult:
    ...
    def load(cls, filepath: str) -> 'HVSRResult':
        ...
        # Reconstruct peaks (JSON uses 'type' key, dataclass uses 'peak_type')
        peaks = []
        for p in data.get('peaks', []):
            pk = dict(p)
            if 'type' in pk:
                pk['peak_type'] = pk.pop('type')
            peaks.append(Peak(**pk))
